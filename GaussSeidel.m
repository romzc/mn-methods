...
if nargin < 5                 
    errordlg('Insuficientes datos de entrada. Lea las instrucciones del código para más información');
elseif nargin > 6
    errordlg('Demasiados datos de entrada. Lea las instrucciones del código para más información');
elseif nargin == 5
    lambda = 1;
else
    ...
    elseif lambda < 0 || lambda > 2
        errordlg('El factor de relajación tiene valores de 0 a 2');
    end
end
...
elseif isempty(Niter) == 0 && isempty(es) == 0
    errordlg('Niter y es no pueden tener un dato de entrada al mismo tiempo, uno de los dos debe estar vacío (ejemplo: Niter = [])');
end
...
for i = 1:Niter - 1
    ...
    if i >= 30
        if any(ea(i+1,:) > es)
            errordlg('Convergencia lenta o divergencia detectada. Use otros valores iniciales');
        end
    end
...
xn = xi(end,:)';
M = [Encabezado ; Datos];
figu = figure('name','Gauss Seidel');
set(figu,'MenuBar', 'none');
set(figu,'ToolBar','none');
ui_table = uitable('columnname', Encabezado,...
                    'data',Datos,...
                    'position',[10 210 510 200]);