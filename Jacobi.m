function [M,xn] = Jacobi(A,B,xi,Niter,es)
...
if nargin < 5                 
    ...
    elseif size(A,1) ~= length(xi)
        errordlg('Se debe ingresar un valor inicial para cada variable del sistema');
    end
end
...
if  isempty(Niter) == 1
    ...
    %errordlg('Solución por error aproximado establecido para converger');
elseif isempty(es) == 1
    metodo = 2; disp(newline);
    %errordlg('Solución por número máximo de iteraciones para converger');
